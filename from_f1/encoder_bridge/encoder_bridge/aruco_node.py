...
class ArucoNode(Node):
    def __init__(self):
        ...
        # Pose-use filter: markers can still be drawn on the debug image,
        # but only clean candidates are published to /aruco_multi_markers.
        ...
        self.get_logger().info(
            f'Target marker ID: disabled ({self.target_marker_id})'
        )
        ...
        self.get_logger().info(
            'Target marker tracking disabled; wall marker ID 98 remains static'
        )
    ...
    def create_marker_candidate(
        ...
        for idx in range(4):
            p1 = points[idx]
            p2 = points[(idx + 1) % 4]
            side_lengths.append(math.hypot(float(p2[0] - p1[0]), float(p2[1] - p1[1])))
        min_side = max(min(side_lengths), 1e-6)
        max_side = max(side_lengths)
        aspect = max_side / min_side
        ...
        min_y = min(float(p[1]) for p in points)
        max_y = max(float(p[1]) for p in points)
        edge_distance = min(min_x, min_y, self.frame_width - max_x, self.frame_height - max_y)
