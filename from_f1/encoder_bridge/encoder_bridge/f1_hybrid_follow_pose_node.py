#!/usr/bin/env python3
# flake8: noqa: E501
"""F1 target follow with TB3 AMCL fallback, one-shot wall-pose recovery, and map return-home.

Main behavior:
- When marker 159 is visible, F1 follows the marker directly.
- When marker 159 is lost, F1 uses TB3 /amcl_pose and F1 /relative_pose to follow a map target behind TB3.
- If TB3 pose or F1 pose is unavailable, F1 falls back to the original wall-marker recovery flow.
"""
...
class F1HybridFollowPoseNode(Node):
    """Follow marker 159, and use TB3 AMCL map-follow when the marker is lost."""
    ...
    def __init__(self):
        ...
        self.mission_enabled = bool(self.get_parameter('start_enabled').value)
        ...
        self.get_logger().info(
            'F1 hybrid node started: target=159, marker follow + TB3 AMCL map fallback enabled'
        )
        self.get_logger().info(
            f'TB3 fallback: follow_distance={self.tb3_follow_distance:.2f}m, '
            f'tb3_pose_timeout={self.tb3_pose_timeout_sec:.2f}s, '
            f'f1_pose_timeout={self.f1_pose_timeout_sec:.2f}s'
        )
    ...
    def _declare_parameters(self):
        values = {
            ...
            # 기존보다 안전하게 수정.
            # 기존 0.12m에서는 15cm에서도 w가 나가서 너무 가까웠음.
            'follow_stop_distance_m': 0.30,
            'follow_forward_distance_m': 0.40,
            'follow_min_align_distance_m': 0.28,
            ...
            # TB3 AMCL fallback.
            'tb3_follow_distance_m': 0.45,
            'tb3_pose_timeout_sec': 1.20,
            'f1_pose_timeout_sec': 1.20,
            'tb3_map_position_tolerance_m': 0.10,
            ...
            'load_hold_min_distance_m': 0.30,
            'load_hold_max_distance_m': 0.45,
    ...
    def _load_parameters(self):
        ...
        self.follow_min_align_distance = float(get('follow_min_align_distance_m'))
        ...
        self.tb3_follow_distance = float(get('tb3_follow_distance_m'))
        self.tb3_pose_timeout_sec = float(get('tb3_pose_timeout_sec'))
        self.f1_pose_timeout_sec = float(get('f1_pose_timeout_sec'))
        self.tb3_map_position_tolerance = float(get('tb3_map_position_tolerance_m'))
        self.tb3_map_yaw_tolerance = float(get('tb3_map_yaw_tolerance_deg'))
        self.tb3_map_bearing_tolerance = float(get('tb3_map_bearing_tolerance_deg'))
        self.tb3_map_hard_bearing = float(get('tb3_map_hard_bearing_deg'))
    ...
    def _load_marker_faces(self, path):
        ...
        try:
            ...
            for marker_id, pose in data.get('aruco_marker_pose', {}).items():
                ...
                faces[int(marker_id)] = int(
                    round(self.normalize(yaw) / (math.pi / 2.0))
                ) % 4

            self.get_logger().info(
                f'Loaded {len(faces)} marker face references from {path}'
            )
    ...
    def signal_true(cls, data):
        ...
        return (
            parsed is not None
            and parsed.get('value', '').lower() in cls.TRUE_VALUES
        )
    ...
    def tb3_amcl_pose_callback(self, msg):
        ...
        self.tb3_pose_time = time.monotonic()
    ...
    def target_callback(self, msg):
        ...
        if seen:
            ...
            self.get_logger().warn(
                f'target marker lost: id={self.target_marker_id}'
            )
    ...
    def pose_callback(self, msg):
        ...
        self.last_pose_source = parsed.get('source', '')

        collecting = self.state == 'POSE_RECOVERY' or (
            self.state == 'RETURN_HOME'
            and self.return_home_recovery_pending
        )
    ...
    def encoder_callback(self, msg):
        ...
        if self.control_pose is None or self.state not in (
            'REACQUIRE_TARGET',
            'RETURN_HOME',
        ):
            return
        ...
        left_distance = (
            -left_delta
            * self.left_encoder_scale
            / self.encoder_counts_per_turn
            * self.wheel_circumference
        )

        right_distance = (
            right_delta
            * self.right_encoder_scale
            / self.encoder_counts_per_turn
            * self.wheel_circumference
        )
    ...
    def consider_recovery_candidate(self, parsed, now):
        ...
        if marker_count < self.recovery_min_markers:
            two_marker_recovery = (
                marker_count == 2
                and self.allow_two_marker_initial
            ...
            if not two_marker_recovery:
                self.log_reject(
                    'count',
                    f'markers={marker_count} < {self.recovery_min_markers}',
                )
                return
        ...
        if reproj > self.recovery_max_reproj:
            self.log_reject(
                'reproj',
                f'reproj={reproj:.2f}px > {self.recovery_max_reproj:.2f}px',
            )
            return
        ...
        group = tuple(
            marker_id
            for marker_id in self.latest_group
        ...
        if self.recovery_group and not self.groups_compatible(
            self.recovery_group,
            group,
        ):
            self.log_reject(
                'group',
                f'non-adjacent marker group {list(group)} vs {list(self.recovery_group)}',
            )
            return
        ...
        required_samples = (
            5
            if self.recovery_two_marker_session
            else self.recovery_sample_count
        )
    ...
    def try_lock_recovery_pose(self):
        ...
        yaw_spread = max(
            abs(math.degrees(self.normalize(yaw - center_yaw)))
            for yaw in yaws
        )
        ...
        if self.state == 'POSE_RECOVERY':
            ...
            self.get_logger().info(
                'return_home pose lock complete; direct home control enabled'
            )
    ...
    def clear_recovery_candidates(self, reason):
        ...
        self.get_logger().info(
            f'recovery candidates cleared: {reason}'
        )
    ...
    def load_done_callback(self, msg):
        ...
        self.get_logger().warn(
            f'load_done received: {msg.data.strip()}'
        )
    ...
    def reanchor_callback(self, msg):
        ...
        if self.state not in ('POSE_RECOVERY', 'RETURN_HOME'):
            self.get_logger().warn(
                f'manual reanchor ignored in state={self.state}'
            )
            return
        ...
        self.get_logger().warn(
            'manual reanchor accepted; one face change is now allowed'
        )
    ...
    def transition(self, new_state, reason):
        ...
        self.get_logger().warn(
            f'STATE changed: {old_state} -> {new_state}; reason={reason}'
        )

        if new_state == 'POSE_RECOVERY':
            self.reset_recovery_lock('target lost recovery')

        if new_state == 'TB3_MAP_FOLLOW':
            self.active_goal = None
            self.control_pose = None
    ...
    def target_is_fresh(self, now):
        return (
            self.target_seen
            and now - self.last_target_seen_time <= self.target_fresh_timeout_sec
        )

    def tb3_pose_is_fresh(self, now):
        return (
            self.tb3_pose is not None
            and now - self.tb3_pose_time <= self.tb3_pose_timeout_sec
        )

    def f1_pose_is_fresh(self, now):
        return (
            self.last_pose_input is not None
            and now - self.last_pose_input_time <= self.f1_pose_timeout_sec
        )

    def tb3_map_follow_available(self, now):
        return self.tb3_pose_is_fresh(now) and self.f1_pose_is_fresh(now)

    def tb3_follow_goal(self):
        tb3_x, tb3_y, tb3_yaw = self.tb3_pose

        goal_x = tb3_x - self.tb3_follow_distance * math.cos(tb3_yaw)
        goal_y = tb3_y - self.tb3_follow_distance * math.sin(tb3_yaw)
        goal_yaw = tb3_yaw
    ...
    def control_loop(self):
        ...
        elif self.state == 'TB3_MAP_FOLLOW':
            self.control_tb3_map_follow(now)
    ...
    def control_follow(self, now):
        if not self.target_is_fresh(now):
            ...
            if now - self.last_target_seen_time >= self.lost_timeout_sec:
                ...
                if self.tb3_map_follow_available(now):
                    self.transition(
                        'TB3_MAP_FOLLOW',
                        'target lost; using TB3 AMCL map fallback',
                    )
                else:
                    self.transition(
                        'TARGET_LOST',
                        'lost_timeout_sec exceeded and TB3/F1 pose unavailable',
                    )
        ...
        # 너무 가까우면 회전/전진 모두 금지.
        # 기존에는 15cm에서도 w/q/e가 나가서 휙 도는 문제가 있었음.
        if distance < self.follow_min_align_distance:
            self.publish_command(
                's',
                f'target too close for safe align {distance:.3f}m',
            )

        elif abs(bearing) > self.follow_hard_bearing:
            self.publish_command(
                's',
                f'target bearing hard limit {bearing:.1f}deg',
            )

        elif abs(bearing) > self.follow_bearing_tolerance:
            self.publish_command(
                'e' if bearing > 0.0 else 'q',
            ...
            self.publish_command(
                'w',
                'target follow forward',
            )

        else:
            self.publish_command(
                's',
                f'target distance hold {distance:.3f}m',
            )
        ...
        if not in_hold:
            ...
            self.transition(
                'LOAD_WAIT',
                'target held at configured load distance',
            )

    def control_tb3_map_follow(self, now):
        if self.target_is_fresh(now):
            self.transition(
                'FOLLOW_TARGET',
                'target marker reacquired during TB3 map fallback',
            )
            return

        if not self.tb3_pose_is_fresh(now):
            self.publish_command('s', 'TB3 AMCL pose timeout')
            self.transition('TARGET_LOST', 'TB3 pose timeout during map fallback')
            return

        if not self.f1_pose_is_fresh(now):
            self.publish_command('s', 'F1 relative pose timeout')
            self.transition('TARGET_LOST', 'F1 pose timeout during map fallback')
            return

        f1_x, f1_y, f1_yaw = self.last_pose_input
        goal_x, goal_y, goal_yaw = self.tb3_follow_goal()

        dx = goal_x - f1_x
        dy = goal_y - f1_y
        distance = math.hypot(dx, dy)

        if distance > self.tb3_map_position_tolerance:
            bearing_error = math.degrees(
                self.normalize(math.atan2(dy, dx) - f1_yaw)
            )

            if abs(bearing_error) > self.tb3_map_hard_bearing:
                self.publish_command(
                    's',
                    f'TB3 map bearing hard limit {bearing_error:.1f}deg',
                )
                return

            if abs(bearing_error) > self.tb3_map_bearing_tolerance:
                cmd = 'q' if bearing_error > 0.0 else 'e'
                self.publish_command(
                    cmd,
                    f'TB3 map align distance={distance:.3f}, bearing={bearing_error:.1f}',
                )
                return

            self.publish_command(
                'w',
                f'TB3 map forward distance={distance:.3f}, bearing={bearing_error:.1f}',
            )
            return

        yaw_error = math.degrees(
            self.normalize(goal_yaw - f1_yaw)
        )

        if abs(yaw_error) > self.tb3_map_yaw_tolerance:
            cmd = 'q' if yaw_error > 0.0 else 'e'
            self.publish_command(
                cmd,
                f'TB3 map final yaw align {yaw_error:.1f}deg',
            )
            return

        self.publish_command(
            's',
            f'TB3 map hold distance={distance:.3f}, yaw_error={yaw_error:.1f}',
        )
    ...
    def control_reacquire(self, now):
        if self.target_is_fresh(now):
            self.transition(
                'FOLLOW_TARGET',
                'target marker reacquired during motion',
            )
            return
        ...
        if (
            not self.recovery_locked
            or self.control_pose is None
        ...
        self.publish_command(
            command,
            'reacquire direct pose control',
        )

        if arrived:
            self.publish_command(
                's',
                'reacquire pose reached; waiting for target',
            )
    ...
    def control_return_home(self):
        if self.return_home_recovery_pending:
            self.publish_command(
                's',
                'return_home recovery pose collection',
            )
            return
        ...
        self.publish_command(
            command,
            'return_home direct pose control',
        )

        if arrived:
            self.transition(
                'DONE',
                'home position and yaw reached',
            )
    ...
    def goal_command(self, goal, position_tolerance, yaw_tolerance_deg):
        ...
        if distance > position_tolerance:
            bearing_error = math.degrees(
                self.normalize(math.atan2(dy, dx) - yaw)
            )
    ...
    def log_reject(self, key, reason):
        ...
        if now - self.last_reject_log.get(key, 0.0) >= 1.0:
            self.get_logger().warn(
                f'recovery pose rejected reason: {reason}'
            )
    ...
    def publish_status(self):
        ...
        tb3 = self.tb3_pose or (float('nan'),) * 3
        f1 = self.last_pose_input or (float('nan'),) * 3

        tb3_goal = (float('nan'), float('nan'), float('nan'))

        if self.tb3_pose is not None:
            tb3_goal = self.tb3_follow_goal()
        ...
        msg.data = (
            ...
            f'target_bearing_deg={self.target_bearing_deg:.1f},'
            f'tb3_pose_fresh={1 if self.tb3_pose_is_fresh(now) else 0},'
            f'f1_pose_fresh={1 if self.f1_pose_is_fresh(now) else 0},'
            f'tb3_x={tb3[0]:.3f},tb3_y={tb3[1]:.3f},tb3_yaw={tb3[2]:.4f},'
            f'f1_x={f1[0]:.3f},f1_y={f1[1]:.3f},f1_yaw={f1[2]:.4f},'
            f'tb3_goal_x={tb3_goal[0]:.3f},tb3_goal_y={tb3_goal[1]:.3f},tb3_goal_yaw={tb3_goal[2]:.4f},'
