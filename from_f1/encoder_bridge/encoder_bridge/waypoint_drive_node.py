...
class WaypointDriveNode(Node):
    def __init__(self):
        ...
        # If /f1/path_points publishes PATH,status=ok, this node follows that path.
        ...
        self.get_logger().info('waypoint_drive_node F1 VARIABLE_PIVOT_ASTAR_OPTIONAL_V5_FIXED_TOPICS started.')
        self.get_logger().info(
            'Mode: PoseStamped direct goal + optional A* path + lookahead + variable pivot + waypoint_cmd output'
        )
        self.get_logger().info(
            'Subscribing: /f1/relative_pose, /f1/goal_pose, /f1/goal_cmd, /f1/path_points, /f1/aruco_marker'
        )
        self.get_logger().info('Publishing: /f1/waypoint_cmd')
        self.get_logger().info(
