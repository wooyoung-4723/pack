#!/usr/bin/env python3
"""Exclusive command router for the F2 unified controller graph."""
...
class F2ModeManagerNode(Node):
    VALID_MODES = ('paused', 'follow', 'waypoint', 'return_home', 'stop')
    VALID_COMMANDS = ('w', 'a', 's', 'c', 'd', 'q', 'e')

    def __init__(self):
        super().__init__('f2_mode_manager_node')
        self.mode = 'paused'
        self.last_output = 's'
        ...
        self.robot_cmd_pub = self.create_publisher(String, '/f2/robot_cmd', 20)
        self.status_pub = self.create_publisher(String, '/f2/mode_status', 10)

        # F2's follow node subscribes to /follow_enable after remapping.
        self.follow_enable_pub = self.create_publisher(
            String, '/f2/follow_enable', 10
        )
        # Keep this for dashboard/common mission compatibility. It is harmless
        # if no F2 node subscribes to it.
        self.mission_enable_pub = self.create_publisher(
            String, '/f2/mission_enable', 10
        )
        self.return_home_pub = self.create_publisher(
            String, '/f2/return_home', 10
        )

        self.create_subscription(String, '/f2/mode', self.mode_callback, 10)
        self.create_subscription(
            String, '/f2/follow_cmd', self.follow_command_callback, 20
        )
        self.create_subscription(
            String, '/f2/waypoint_cmd', self.waypoint_command_callback, 20
        )
        ...
        self.get_logger().info('F2 mode manager started in paused mode')
    ...
    def mode_callback(self, msg):
        ...
        if requested not in self.VALID_MODES:
            self.get_logger().warn(f'Invalid F2 mode ignored: {requested}')
            return

        previous = self.mode
        self.mode = requested
        self.publish_robot_command('s', f'mode change {previous}->{requested}')
        ...
        if requested == 'return_home':
            # F2 return-home can be connected later. For now, publish the event
            # and keep the robot stopped instead of letting another controller
            # drive /f2/robot_cmd directly.
            self.return_home_pending_at = time.monotonic() + 0.2
        else:
            self.return_home_pending_at = None

        self.get_logger().warn(f'F2 mode changed: {previous} -> {requested}')
        self.publish_status()
    ...
    def set_follow_enabled(self, enabled):
        data = 'start' if enabled else 'stop'
        self.publish_text(self.follow_enable_pub, data)
        self.publish_text(self.mission_enable_pub, data)
    ...
    def safety_timer(self):
        ...
        if self.mode in ('paused', 'stop', 'return_home'):
