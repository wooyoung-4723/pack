...
class F1ModeManagerNode(Node):
    ...
    def __init__(self):
        ...
        self.robot_cmd_pub = self.create_publisher(String, '/f1/robot_cmd', 20)
        self.status_pub = self.create_publisher(String, '/f1/mode_status', 10)
        self.hybrid_enable_pub = self.create_publisher(
            String, '/f1/mission_enable', 10
        )
        self.return_home_pub = self.create_publisher(
            String, '/f1/return_home', 10
        )

        self.create_subscription(String, '/f1/mode', self.mode_callback, 10)
        self.create_subscription(
            String, '/f1/follow_cmd', self.follow_command_callback, 20
        )
        self.create_subscription(
            String, '/f1/waypoint_cmd', self.waypoint_command_callback, 20
        )
        ...
        self.get_logger().info('F1 mode manager started in paused mode')
    ...
    def mode_callback(self, msg):
        ...
        if requested == 'return_home':
            # Allow the hybrid node to process mission_enable first.
    ...
    def set_hybrid_enabled(self, enabled):
        self.publish_text(
            self.hybrid_enable_pub, 'start' if enabled else 'stop'
        )
